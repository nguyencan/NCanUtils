...
open class CNImageViewerController: UIViewController {
    ...
    public let imageInfo: CNImageInfo
    
    open var transitionInfo: CNTransitionInfo?
    ...
    public init(imageInfo: CNImageInfo) {
        self.imageInfo = imageInfo
        super.init(nibName: nil, bundle: nil)
    }
    
    public convenience init(imageInfo: CNImageInfo, transitionInfo: CNTransitionInfo) {
        ...
        if let fromView = transitionInfo.fromView, let referenceView = fromView.superview {
            ...
            if fromView.contentMode != imageInfo.contentMode {
                transitionInfo.convertedRect = imageInfo.calculate(
                    ...
                    imageMode: CNImageInfo.ImageMode(rawValue: fromView.contentMode.rawValue)
                )
            } else {
    ...
    public convenience init(image: UIImage, imageMode: UIView.ContentMode, imageHD: URL?, fromView: UIView?) {
        let imageInfo = CNImageInfo(image: image, imageMode: CNImageInfo.ImageMode(rawValue: imageMode.rawValue)!, imageHD: imageHD)
        
        if let fromView = fromView {
            self.init(imageInfo: imageInfo, transitionInfo: CNTransitionInfo(fromView: fromView))
        } else {
            self.init(imageInfo: imageInfo)
        }
    }
...
extension CNImageViewerController: UIViewControllerTransitioningDelegate {
    ...
    public func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return CNImageViewerTransition(imageInfo: imageInfo, transitionInfo: transitionInfo!, transitionMode: .dismiss)
    }
    
}

class CNImageViewerTransition: NSObject, UIViewControllerAnimatedTransitioning {
    
    let imageInfo      : CNImageInfo
    let transitionInfo : CNTransitionInfo
    ...
    init(imageInfo: CNImageInfo, transitionInfo: CNTransitionInfo, transitionMode: TransitionMode) {
        self.imageInfo = imageInfo
        self.transitionInfo = transitionInfo
